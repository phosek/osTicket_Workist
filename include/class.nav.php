...
class AdminNav extends StaffNav{
    ...
    function getTabs(){
        ...
        if(!isset($this->tabs)){
            ...
            $tabs['custom']=array('desc'=>__('Custom settings'),'href'=>'workist_settings.php','title'=>__('Custom settings'));
            if (!is_null($this->getRegisteredApps()))
        ...
        foreach($this->getTabs() as $k=>$tab){
            ...
            switch(strtolower($k)){
                ...
                case 'apps':
                    ...
                    break;
                case 'custom':
                    $subnav[]=array('desc'=>__('Workist'),'href'=>'workist_settings.php','iconclass'=>'email-settings');
                    break;
            }
            if($subnav)
                $submenus[$this->getPanel().'.'.strtolower($k)]=$subnav;
